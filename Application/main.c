...
void Timer_ISR(void)
{
    ...
    else if ((ball.x <= left_boundary + boundary_thick + paddle_width && ball.y + ball_size >= paddle1_pos && ball.y <= paddle1_pos + paddle_height) ||
        (ball.x + ball_size >= right_boundary - paddle_width && ball.y + ball_size >= paddle2_pos && ball.y <= paddle2_pos + paddle_height))
    {
